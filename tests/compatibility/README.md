...
Bootstrap copies editable prompt templates once to `/srv/b1-ai-hub/workflows/acceptance/` and preserves later operator edits. `make native-comfyui-compatibility` defaults to the checked-in `workflows/acceptance/native-comfyui-smoke-prompt.json` when `B1_NATIVE_COMFYUI_PROMPT_FILE` is not set; it emits a deterministic tiny image through the B1 runtime hook and is useful for route-level rehearsals without model weights. Final native compatibility handoff should set `B1_NATIVE_COMFYUI_PROMPT_FILE` to `native-comfyui-upload-save-prompt.json` or another standard-node native prompt that does not use `B1RuntimeTinyImage`. The separate GPU/image acceptance harness still uses the edited `text-to-image-api-prompt.json` with installed model weights.
...
export B1_NATIVE_COMFYUI_PROMPT_FILE=/srv/b1-ai-hub/workflows/acceptance/native-comfyui-upload-save-prompt.json
export B1_NATIVE_COMFYUI_EVIDENCE=/srv/b1-ai-hub/backups/acceptance/native-comfyui.json
make native-comfyui-compatibility
```

For a quick route-level rehearsal, omit `B1_NATIVE_COMFYUI_PROMPT_FILE` or set it to `workflows/acceptance/native-comfyui-smoke-prompt.json`. When a prompt contains stock `LoadImage` nodes, the harness pre-uploads each referenced plain image filename as a tiny PNG before submission; filenames with paths, traversal, control characters, or unsupported extensions are rejected before any upload. For temporary IP/host validation, set `B1_NATIVE_COMFYUI_HOST_HEADER=comfy.ai.b1.germering` and `B1_NATIVE_COMFYUI_API_HOST_HEADER=api.ai.b1.germering`. For a Caddy internal CA that is not trusted by the test host yet, set `B1_NATIVE_COMFYUI_CA_FILE=/path/to/root.crt`; use `B1_NATIVE_COMFYUI_TLS_VERIFY=0` only during an explicit LAN validation window. The harness refuses to send `B1_NATIVE_COMFYUI_API_KEY` over plain HTTP or `ws://` unless `B1_ACCEPTANCE_ALLOW_INSECURE_HTTP=true` is set for an isolated development run. `B1_NATIVE_COMFYUI_PROMPT_JSON` may be used instead of `B1_NATIVE_COMFYUI_PROMPT_FILE` for a small inline native prompt.

When `B1_NATIVE_COMFYUI_EVIDENCE` is set, the test writes a machine-readable evidence file with `status`, prompt metadata, `required_checks`, per-check records, and redacted route/prompt samples. The prompt metadata records only the source, filename, node/class counts, class types, and whether it used `B1RuntimeTinyImage`; it does not record node inputs or prompt text. Control Center acceptance reports ingest the latest supported direct-child `$B1_BACKUP_ROOT/acceptance/*.json` native ComfyUI evidence file and block handoff if the evidence uses the tiny route-level smoke prompt, or if `/object_info`, `/object_info/{node}`, `/system_stats`, `/models`, `/queue`, `/upload/image`, `/upload/mask`, `POST /prompt`, `Idempotency-Key` replay, `/ws`, `/history`, `/history/{prompt_id}`, B1 durable job lookup by native prompt ID, B1 artifact listing/download, `POST /queue` deletion, targeted `POST /interrupt`, or `/view` artifact checks are absent or incomplete. The report also requires native prompt ID correlation, replay header proof, completed native WebSocket events or previews, B1 durable job/artifact IDs, successful queue/interrupt HTTP status, B1 `artifact_proofs[]` entries for every stored artifact, and retrieved `/view` proof entries with byte count, SHA-256, content type, filename, and output key for every returned native history artifact; check names alone are not accepted as proof.
...
The legacy harness intentionally sends no bearer token. The listener strips any client `Authorization`, `Cookie`, or `X-B1-CSRF` headers before forwarding to the scheduler-aware Caddy/control-plane path, then verifies that `/object_info`, `/system_stats`, and `/ws` are reachable. Its evidence records the legacy profile name, publish mapping, CIDR allowlist, compatibility marker, scheduler-aware gateway target, no-direct-backend assertion, and per-route proof that no bearer/cookie/CSRF headers were sent. Control Center acceptance reports ingest this optional evidence when present; missing legacy evidence is non-blocking because the listener is disabled by default, but incomplete, stale, open-world allowlist, direct-backend, or shallow check-name-only evidence is reported. It is not a substitute for the authenticated native ComfyUI compatibility evidence required for handoff.
The default `B1_LEGACY_COMFY_PUBLISH=8188:8188` follows the host's DHCP-owned addresses instead of hard-coding a LAN IP. Keep that mapping for final evidence. If you use a temporary emergency host-IP override, remove it before acceptance evidence is generated and keep the CIDR allowlist narrow throughout.

## Remote Nodes Without Server-Side ComfyUI

The first concrete remote-node scenario is the non-Comfy smoke path: run `integrations/comfyui-b1-remote-nodes/examples/tts-fast.non-comfy.workflow.json` from an external ComfyUI while the server-side B1 `comfyui` container is stopped. The package must register the full required B1 node surface, the checked-in example workflows must cover those node classes without credential material, the workflow must list/select B1 models through the unified API, and execution must complete through the `audio-cpu`/`tts-fast` path.
