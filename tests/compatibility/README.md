...
The native ComfyUI compatibility path uses the public `comfy.ai.b1.germering` gateway endpoint, not the internal `comfyui` container port. It verifies metadata routes, native prompt submission, native WebSocket events, native history lookup, and `/view` artifact retrieval with a real API-format prompt supplied by the operator.
