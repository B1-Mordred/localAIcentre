...
class ArtifactCleanupApiTests(unittest.TestCase):
    ...
    def write_sample(self, root: Path, relative: str, content: bytes, age_days: int) -> Path:
        target = root / relative
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_bytes(content)
        timestamp = (datetime.now(tz=UTC) - timedelta(days=age_days)).timestamp()
    ...
    def test_voicebox_sample_retention_plan_preserves_profile_references(self) -> None:
        class SampleDatabase(FakeArtifactDatabase):
            async def list_voice_profiles(
                ...
                return [{"id": "vp_1", "sample_artifacts": [{"url": "/artifacts/voicebox/references/operator_1/sample_keep/keep.wav"}]}]

        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            fake_database = SampleDatabase(root)
            self.write_sample(root, "voicebox/references/operator_1/sample_old/old.wav", b"old", 45)
            self.write_sample(root, "voicebox/references/operator_1/sample_keep/keep.wav", b"keep", 45)
            self.patch_common(fake_database, [])

            plan = asyncio.run(
                main.admin_voicebox_sample_retention_plan(
                    main.VoiceboxSampleRetentionRequest(delete_older_than_days=30, limit=100)
                )
            )

            self.assertEqual(plan["candidate_count"], 1)
            self.assertEqual(plan["candidates"][0]["path"], "voicebox/references/operator_1/sample_old/old.wav")
            self.assertEqual(plan["kept"][0]["reason"], "protected_by_voice_profile")

    def test_voicebox_sample_cleanup_requires_confirmation_deletes_and_audits(self) -> None:
        audit_events: list[dict[str, Any]] = []
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            fake_database = FakeArtifactDatabase(root)
            sample = self.write_sample(root, "voicebox/references/operator_1/sample_old/old.wav", b"old", 45)
            self.patch_common(fake_database, audit_events)

            with self.assertRaises(HTTPException) as missing_confirmation:
                asyncio.run(
                    main.admin_voicebox_sample_cleanup(
                        main.VoiceboxSampleRetentionRequest(delete_older_than_days=30, confirm=False)
                    )
                )
            self.assertEqual(missing_confirmation.exception.status_code, 400)
            ...
            result = asyncio.run(
                main.admin_voicebox_sample_cleanup(
                    main.VoiceboxSampleRetentionRequest(delete_older_than_days=30, confirm=True)
                )
            )

            self.assertEqual(result["deleted_count"], 1)
            self.assertFalse(sample.exists())
            self.assertEqual(audit_events[0]["event_type"], "voice_profile.sample_cleanup")
            self.assertEqual(audit_events[0]["metadata"]["deleted_paths"], ["voicebox/references/operator_1/sample_old/old.wav"])
