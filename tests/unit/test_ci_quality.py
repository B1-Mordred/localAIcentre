...
class CiQualityGateTests(unittest.TestCase):
    ...
    def test_makefile_exposes_live_acceptance_evidence_targets(self) -> None:
        ...
        self.assertIn('B1_ACCEPTANCE_REPORT_OUTPUT ?= $(B1_BACKUP_ROOT)/acceptance/operator-handoff-report-response.json', self.makefile_text)
        self.assertIn("\nacceptance-report-preview:", self.makefile_text)
        self.assertIn("B1_ACCEPTANCE_REPORT_REQUIRE_READY=false python3 deploy/scripts/acceptance_report.py preview", self.makefile_text)
        self.assertIn("\noperator-handoff-report:", self.makefile_text)
        self.assertIn("python3 deploy/scripts/acceptance_report.py create", self.makefile_text)
        self.assertNotIn("--api-key", self.makefile_text)
