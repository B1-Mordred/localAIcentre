...
class NetworkDhcpPlanTests(unittest.TestCase):
    def test_static_technitium_address_is_preserved_and_dhcp_reservation_is_required(self) -> None:
        ...
        self.assertEqual(plan["host_infrastructure_static_addresses"][0]["assignment"], "static-on-interface")
        self.assertTrue(plan["host_infrastructure_static_addresses"][0]["must_remain_on_interface"])
        self.assertTrue(plan["safety"]["static_host_infrastructure_required_on_interface"])
        self.assertTrue(plan["safety"]["static_host_infrastructure_preserved_on_candidate"])
        self.assertTrue(plan["static_infrastructure_policy"]["required_on_active_interface"])
        self.assertTrue(plan["static_infrastructure_policy"]["candidate_preserves_all_static_addresses"])
