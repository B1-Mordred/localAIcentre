...
class AcceptanceEnvTests(unittest.TestCase):
    def test_render_contains_live_flags_paths_and_blank_operator_values(self) -> None:
        ...
        self.assertIn('export OLD_STACK_BACKUP="${OLD_STACK_BACKUP:-}"', text)
        self.assertIn('export OPEN_WEBUI_PLAN="${OPEN_WEBUI_PLAN:-}"', text)
        self.assertIn('export CUTOVER_PLAN="${CUTOVER_PLAN:-}"', text)
        self.assertIn('export ROLLBACK_REPORT="${ROLLBACK_REPORT:-$B1_BACKUP_ROOT/rollback-rehearsal.json}"', text)
    ...
    def test_render_uses_configured_chat_host_for_open_webui_smoke(self) -> None:
        ...
        self.assertIn('export B1_HOST_CHAT="${B1_HOST_CHAT:-chat.test.lan}"', text)
