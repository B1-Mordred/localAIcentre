...
EVIDENCE_FILES = {
    ...
    "B1_MODEL_TOOLS_EVIDENCE": "model-tools.json",
...
def render_acceptance_env(config: AcceptanceEnvConfig) -> str:
    ...
    lines.append("# B1 defines the appliance system hostname; IP, gateway, and resolver properties come from DHCP.")
    lines.append(shell_default_literal("B1_HOST_CHAT", config.host_chat))
    lines.append(shell_default_expression("B1_APPLIANCE_HOSTNAME", "$B1_HOST_CHAT"))
    lines.append(shell_default_expression("B1_EXPECTED_TARGET_HOST", "$B1_APPLIANCE_HOSTNAME"))
    lines.append(shell_default_literal("B1_NETWORK_DHCP_PLAN", str(backup_root / "network-dhcp-plan.json")))
    lines.append(shell_default_literal("B1_DHCP_REQUIRED_ADDRESSES", "192.168.2.100=b1-ai-hub-gateway"))
    lines.append(shell_default_literal("B1_STATIC_INFRASTRUCTURE_ADDRESSES", "192.168.2.2/24=technitium-dhcp-dns"))
    lines.append(shell_default_literal("B1_DHCP_RESERVATION_CONFIRMED", "0"))
