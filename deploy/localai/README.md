...
Laguna XS 2.1 keeps tokenizer Jinja enabled and explicitly enables thinking in `chat_template_kwargs`. GPT-OSS 20B likewise enables both LocalAI's structured tokenizer-template path and the embedded Harmony Jinja template; setting only `use_jinja` leaves chat messages as an unformatted completion prompt. B1 forwards the selected `reasoning_effort` (`low`, `medium`, or `high`) to LocalAI, which supplies it to Harmony's native template kwarg. The unified chat path also preserves OpenAI-compatible `reasoning_content`, `tool_calls`, tool response messages, and top-level tool fields when forwarding to LocalAI, so agent loops can retain the full reasoning/tool history required by those model families.
