...
`native-comfyui-upload-save-prompt.json` uses stock ComfyUI `LoadImage` and `SaveImage` nodes. The native compatibility harness pre-uploads the referenced tiny input image before `POST /prompt`, so this template proves the authenticated gateway/proxy, scheduler handoff, native WebSocket, history, `/view`, and durable B1 artifact correlation without requiring installed checkpoint weights. It is valid for the native ComfyUI compatibility evidence only; it does not replace the GPU/image-generation acceptance templates below.
