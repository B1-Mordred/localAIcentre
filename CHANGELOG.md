...
- Added machine-readable native ComfyUI compatibility evidence ingestion and an opt-in live REST/WebSocket harness so handoff now requires proof of `/object_info`, `/system_stats`, `/models`, `/queue`, native queue deletion, targeted interrupt, `/upload/image`, `/upload/mask`, native `POST /prompt`, `/ws`, and `/history/{prompt_id}` through the gateway.
