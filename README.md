...
- native ComfyUI `POST /prompt` admission through the global GPU lease with durable `native_prompt_id` recording, background history tracking, bidirectional `/ws` text/binary bridging, progress parsing, ComfyUI output ingestion into authenticated artifact storage, and `recovery_required` handling when native history produces no B1-manageable media output
