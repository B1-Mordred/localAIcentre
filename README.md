...
- GitHub Actions quality gates for backend tests, Compose validation, Alembic packaging, frontend builds, NPM audit, source secret scanning, CycloneDX SBOM generation/validation, Python dependency audit, service/frontend/integration container builds, strict Trivy scans for the smaller B1-owned images, and uploaded Trivy inventories for upstream-heavy Open WebUI and ComfyUI images
...
- production LocalAI Compose override that replaces the placeholder `localai` service with the official pinned CUDA 12 LocalAI image, resets the inherited mock build fields, mounts only the read-only LocalAI model view plus LocalAI writable state, and moves the control plane to LocalAI's native `:8080` API
- production ComfyUI Compose override and reproducible B1 image build from upstream ComfyUI `v0.3.77` commit `59afc3984868289f808d02fa5cd180edfb2de240`, with native `:8188` API behind the scheduler-aware proxy, read-only runtime model views, disabled API/cloud nodes, explicit input/output/temp/user dirs, and RTX 3060 VRAM reserve defaults
...
LocalAI and ComfyUI now have production runtime overrides:
...
The base Compose file remains the lightweight development topology; these overrides must be used for real GPU runtime smoke and acceptance work. See [deploy/localai/README.md](./deploy/localai/README.md), [deploy/comfyui/README.md](./deploy/comfyui/README.md), and [docs/installation.md](./docs/installation.md).
