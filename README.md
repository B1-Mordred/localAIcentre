...
- database-backed model install planning, resumable direct-url and Hugging Face repository blob downloads, verified staged-blob publication, hardlinked or safely extracted per-runtime read-only model views, recoverable record/blob quarantine with confirmed quarantine retention cleanup, and catalog overlay refresh
