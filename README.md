...
- configurable media-job queue/rate admission, artifact-storage headroom checks, and production GPU hardware/resource-policy admission exposed through `GET /admin/admission`, persisted admin policy APIs, Dashboard, Storage, and System tab editing, returning HTTP 429, 503, or 507 before work is accepted when limits or safety policy would be exceeded
