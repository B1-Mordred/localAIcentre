...
- `tts-fast.non-comfy.workflow.json` submits `B1 Text To Speech` against the `tts-fast` alias. This is the default smoke path for proving the external ComfyUI nodes call the unified API and do not require the server-side B1 ComfyUI runtime; its `proof_json` output records the non-placeholder header and local artifact proof.
- `text-to-image.async.workflow.json` submits an asynchronous `image-default` job, waits for completion, lists artifacts, and downloads the generated artifact through the authenticated `/artifacts/...` route. `B1 Download Artifact` returns `proof_json` with the local path, byte count, digest, and private-file/path-containment checks.
