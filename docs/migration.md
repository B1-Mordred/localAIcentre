...
- host identity and `migration_readiness.target_identity`, including hostname, FQDN, expected appliance hostname/FQDN, OS/kernel, architecture, Python version, UID/GID, and inventory user, so operators can prove the report came from the intended target before cutover
- observed host networking, including interfaces, non-loopback addresses, default routes, DHCP-route evidence, and explicit policy that the hostname is system-owned while B1 AI Hub does not configure host static IP addresses
