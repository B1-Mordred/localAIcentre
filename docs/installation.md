...
The machine hostname is defined by the host system, not by B1 AI Hub. `B1_EXPECTED_TARGET_HOST` is validation evidence for the intended appliance hostname/FQDN, while `B1_HOST_*` values are Caddy virtual hosts and API URLs only. Do not configure a static host IP in the repository or Compose project; acquire address, gateway, and resolver properties through the host DHCP client or a DHCP reservation, then point LAN DNS records for the B1 virtual hosts at that assigned address.
