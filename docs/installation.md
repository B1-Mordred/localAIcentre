...
Open WebUI starts in local-only mode: its global provider/config table is ignored so imported old settings remain recoverable but cannot override the B1 wrapper policy. Ollama, web search, local web fetch, community sharing, evaluation arena models, version checks, OpenTelemetry, and automatic RAG/Whisper model downloads are disabled. The default chat model is `chat-default`, RAG embeddings use `embedding-default`, TTS uses `tts-fast`, and STT uses `stt-default`; manage those public aliases in Control Center and override only the initial Open WebUI defaults with the `B1_OPEN_WEBUI_*` values in `.env` when necessary. Open WebUI's built-in image generator is disabled because B1's image and video workflows are asynchronous and are managed through Media Studio.
