...
B1 configuration defines the intended appliance hostname/FQDN through `B1_EXPECTED_TARGET_HOST` and the LAN virtual-host values. Set the target OS hostname/FQDN to match that configured name before cutover, but do not configure a static host IP in the repository or Compose project; acquire address, gateway, and resolver properties through the host DHCP client or a DHCP reservation, then point LAN DNS records for the B1 virtual hosts at that assigned address.
