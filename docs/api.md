...
Long-running media operations are represented as durable jobs in PostgreSQL. `POST /v1/media/jobs`, `POST /v1/images/generations`, and `POST /v1/images/edits` support `Idempotency-Key`; repeated requests from the same API client with the same key return the original job instead of creating a duplicate. Keys are opaque visible header strings up to 256 characters. Reusing a key for a different job identity returns HTTP 409 with `detail.code=idempotency_key_conflict` and a list of mismatched public fields, never the original prompt, media, or other sensitive request values. Multipart image-edit replays short-circuit before upload staging when the existing key already belongs to an image-edit job.
