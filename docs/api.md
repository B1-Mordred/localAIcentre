...
External adapters are disabled unless `B1_ALLOW_EXTERNAL_PROVIDERS=true` and remain non-selectable unless their configured base URL passes the external-runtime URL policy. The policy requires HTTPS, rejects embedded credentials, query strings, fragments, relative path segments, malformed ports, private/loopback/link-local/reserved IP literals, hostnames that resolve to non-public IP addresses, and hostnames that cannot be resolved safely. `B1_OPENAI_COMPATIBLE_BASE_URL` may point at either an API root or a `/v1` base; OpenAI-style forwarding avoids duplicating `/v1`. Public adapter records include `configured` and `configuration_error` but never expose API keys.
