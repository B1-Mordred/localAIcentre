...
`POST /admin/models/download-plan` uses the same model-or-manifest payload and reports whether the current worker can stage the manifest's blobs. The first worker intentionally supports only HTTPS `direct-url` manifests, no redirects, no embedded username/password, and no common credential query parameters such as `token`, `api_key`, or `signature`. Single-file manifests use `source.url` as the exact file URL. Multi-file manifests use `source.url` as a base URL ending in `/`; each safe manifest `files[].path` is appended and query strings/fragments on the base URL are rejected.
...
The control plane has a versioned runtime-adapter registry for `localai`, `comfyui`, `voicebox`, `audio-cpu`, `openai-compatible`, and `generic-http`. `GET /admin/runtimes` returns the configured adapter capabilities and current health for enabled runtimes. If an adapter returns JSON with a body-level `status`, the control plane surfaces that status; for example, a policy-disabled scaffold `audio-cpu` runtime reports `unconfigured` with `details.capabilities` set to `false` for speech, transcription, and embeddings.
