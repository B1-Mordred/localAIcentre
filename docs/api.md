...
Backup/migration/rollback acceptance also includes a detailed proof summary. The report validates B1 backup file counts, PostgreSQL dump coverage, B1 and old-stack archive SHA-256 values, alternate-restore file counts, Open WebUI backed-up readable account/chat/settings/document domains, cutover DNS/hardware/GPU-runtime/runtime-agent readiness, preserved rollback resources, and rollback rehearsal cutover-plan SHA-256 plus rollback command/action counts before setting `backup_migration_rollback_evidence_ready=true`.
