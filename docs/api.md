...
In `B1_RUNTIME_DEPLOYMENT_MODE=production`, GPU-backed media jobs also require the live `hardware:resource-policy` verdict to be `ok` before a durable row is inserted. If runtime-agent metrics are unavailable, the largest visible GPU is below the effective `gpu_total_vram_gib`, host RAM is below `host_total_ram_gib`, or live free VRAM/RAM is below the configured reserves, admission returns HTTP 503 with `detail.code=hardware_resource_policy` and the redacted check payload. CPU-only jobs do not require GPU telemetry and continue to use only queue/rate/storage admission.
