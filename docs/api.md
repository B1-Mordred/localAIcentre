...
`GET /admin/migration/backup-migration-rollback-evidence` returns readiness for the final backup/migration/rollback evidence bundle: latest valid B1 backup, matching restore report, old-stack inventory, old-stack backup, Open WebUI migration plan, cutover plan, rollback rehearsal report, and current output file. `POST /admin/migration/backup-migration-rollback-evidence` requires administrator `admin:write` and `confirm_reviewed=true`, verifies the selected artifacts, requires the cutover plan's core DNS readiness to be accepted, and writes only `$B1_BACKUP_ROOT/acceptance/backup-migration-rollback.json`. The endpoint auto-selects known artifacts from configured roots and never accepts arbitrary host paths.
