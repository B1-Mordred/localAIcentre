...
Optional filters are `event_type`, `actor_id`, and `target_type`. Audit events record actor ID, role, API-key prefix where applicable, event type, target, summary, correlation ID when available, and redacted metadata. The metadata redactor recursively removes token, secret, prompt, message, upload, voice, image, audio, video, and document fields before storage, and it scans otherwise safe string fields for bearer tokens, B1 API keys, GitHub PAT-shaped strings, and credential query parameters. Admin job mutation reasons are recorded only as `reason_provided` metadata. The audit table is included in the control-plane logical PostgreSQL export and native PostgreSQL dump used by backups.
