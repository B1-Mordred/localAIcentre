...
Native ComfyUI compatibility evidence includes `/object_info`, `/system_stats`, `/models`, `/queue`, `POST /prompt`, `/ws`, `/history/{prompt_id}`, and `/view` retrieval for at least one generated image, video, GIF, or audio artifact.
